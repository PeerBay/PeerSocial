                    ...
                    if (src !== scope.imgSrc) {
                        ...
                        img.onload = function() {
                            ...
                            if (width < scope.width) {
                                ...
                                // scope.destinationWidth=width
                            }
                            minHeight = Math.min(scope.height, scope.destinationHeight);

                            if (height < minHeight) {
                                scope.destinationHeight=height
                ...
                scope.ok = function($event) {
                    ...
                    return $q.all([getCropPromise(), getOriginalPromise()]).then(function(blobArray) {
                        ...
                        for(i in scope.files){
                          if(scope.files[i].lfProperty==scope.property){
                            scope.files.splice(i,i+1)
                          }
                        }
                        readAsDataURL(result.croppedImage).then(function(dataurl) {
                            scope.files.push({
                                "lfFile": result.croppedImage,
                                "lfDataUrl": dataurl,
