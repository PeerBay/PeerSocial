/*globals PouchDB: true, call: false, ajax: true */
...
// Implements the API for dealing with a PouchDBDB peer's database over WebRTC
var PeerPouchDB = function(opts, callback) {
    function TODO(callback) {
        // TODO: callers of this function want implementations
        if (callback) setTimeout(function() {
            callback(PouchDB.Errors.NOT_IMPLEMENTED);
        }, 0);
    }

    // var _init = PeerPouchDB._shareInitializersByName[opts.name];
    // if (!_init) throw Error("Unknown PeerPouchDB share dbname"); // TODO: use callback instead?
    // console.log(_init)
    var handler = new exit('ws://127.0.0.1:8080/ws'),
        api = {}; // initialized later, but PouchDB makes us return this before it's ready
    console.log(handler)
    handler.onconnection = function() {
        console.log(handler)
        opts.callback(handler)
        var rpc = new RPCHandler(handler);
...
PeerPouchDB._wrappedAPI = function(db) {
    /*
...
PeerPouchDB.destroy = function(name, callback) {
    if (callback) setTimeout(function() {
        callback(PouchDB.Errors.FORBIDDEN);
    }, 0);
};

// Can we breathe in this environment?
PeerPouchDB.valid = function() {
...
var _t = PeerPouchDB._types; // local alias for brevitation…

// Register for our scheme
PouchDB.adapter('webrtc', PeerPouchDB);
...
function RPCHandler(tube) {
    ...
    this._callRemote = function(fn, args) {
        ...
        if (window.mozRTCPeerConnection) messages.forEach(function(msg) {
            tube.send(msg);
        });
        else processNext();
...
var SharePouchDB = function(hub) {
    // NOTE: this plugin's methods are intended for use only on a **hub** database
    console.log(hub, this)
        // hub=new PouchDB("http://localhost:5984/hub")
        // var db=this
        ...
        if (watcherCount > 0 && !changesListener) { // start listening for changes (at current sequence)
            ...
            hub.info(function(e, d) {
                ...
                opts.onChange = function(d) {
                    ...
                    if (watchers) watchers.forEach(function(cb) {
                        cb(d.doc);
                    });
                };
        ...
        share._signalWatcher = addWatcher(_t.signal, function receiveSignal(signal) {
            ...
            if (!handler) {
                ...
                handler.onconnection = function() {
                    ...
                    rpc.bootstrap({
                        api: PeerPouchDB._wrappedAPI(db)
                    });
                };
            }
            ...
            hub.post({
                ...
                if (e) console.warn("Couldn't clean up signal", e);
            });
        });
    ...
    function _localizeShare(doc) {
        ...
        if (doc._deleted) delete PeerPouchDB._shareInitializersByName[name];
        else PeerPouchDB._shareInitializersByName[name] = function(opts) {
    ...
    function getShares(opts, cb) {
        ...
        if (opts.onChange) { // WARNING/TODO: listener may get changes before cb returns initial list!
            return addWatcher(_t.share, function(doc) {
                if (!_isLocal(doc)) opts.onChange(_localizeShare(doc));
            });
        }
    }
...
PeerPouchDB._shareInitializersByName = Object.create(null); // global connection between new PeerPouchDB (client) and source SharePouchDB (hub)

SharePouchDB._delete = function() {}; // blindly called by PouchDB.destroy

// if (typeof window !== 'undefined' && window.PouchDBDB) {
//   window.PouchDBDB.adapter('peerba', module.exports);
// }
PouchDB.plugin({
    'hub': SharePouchDB
});
...
PouchDB.dbgPeerPouchDB = PeerPouchDB;
